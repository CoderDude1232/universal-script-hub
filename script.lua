local USE_CUSTOM_GUI = true -- set false to instantly revert to Rayfield

local function BuildAxiomRayfield()
    --========================
    -- Imports / Services
    --========================
    ...
    local LUCIDE_URL = "https://raw.githubusercontent.com/latte-soft/lucide-roblox/refs/heads/master/lib/Icons.luau"
    local _LUCIDE_CACHE = nil

    local function getLucideMap()
        if _LUCIDE_CACHE then return _LUCIDE_CACHE end
        local ok, src = pcall(function() return game:HttpGet(LUCIDE_URL) end)
        ...
        _LUCIDE_CACHE = (ok2 and type(tbl) == "table") and tbl or {}
        return _LUCIDE_CACHE
    end
    ...
    local function resolveIcon(icon)
        -- returns: imageId (number), rectSize Vector2, rectOffset Vector2 OR nil
        ...
        -- rbxassetid://123
        local idFromUrl = icon:match("rbxassetid://(%d+)")
        if idFromUrl then
            return tonumber(idFromUrl), nil, nil
        end

        -- "123"
        if isDigits(icon) then
            return tonumber(icon), nil, nil
        end

        -- lucide name (e.g. "info", "rewind", "map-pin")
        local lucide = getLucideMap()
        local entry = lucide[icon]
        if not entry then return nil end

        -- entry format: { imageId, {w,h}, {ox,oy} }
        local imageId = entry[1]
        local rs = entry[2]
        local ro = entry[3]
        if type(imageId) ~= "number" or type(rs) ~= "table" or type(ro) ~= "table" then
            return nil
        end

        local rectSize = Vector2.new(rs[1] or 0, rs[2] or 0)
        local rectOffset = Vector2.new(ro[1] or 0, ro[2] or 0)
        return imageId, rectSize, rectOffset
    end

    local function makeIcon(parent, icon, sizePx, color)
        sizePx = sizePx or 16
        color = color or Color3.fromRGB(235, 235, 235)

        local imageId, rectSize, rectOffset = resolveIcon(icon)
        ...
        -- lucide atlas rect
        if rectSize and rectSize.X > 0 then
    ...
    --========================
    -- Subtle Glass Styling (DARK + READABLE)
    --========================
    local THEME = {
        Backdrop = Color3.fromRGB(12, 12, 14), -- screen background (blocks world)
        Window   = Color3.fromRGB(20, 20, 24), -- main window
        Panel    = Color3.fromRGB(24, 24, 28), -- cards/rows
        Panel2   = Color3.fromRGB(28, 28, 34), -- inputs/buttons background
        Stroke   = Color3.fromRGB(255, 255, 255),

        Title    = Color3.fromRGB(245, 245, 245),
        Body     = Color3.fromRGB(210, 210, 210),
        Sub      = Color3.fromRGB(170, 170, 170),
    }

    -- Subtle glass hint = small transparency + very soft stroke (NO gradient)
    local function applySubtleGlass(frame, cornerRadiusPx, strokeWidth)
        frame.BackgroundTransparency = 0.08 -- slight glass, still dark
        frame.BorderSizePixel = 0
        make("UICorner", { CornerRadius = UDim.new(0, cornerRadiusPx or 12) }, frame)

        local stroke = make("UIStroke", {}, frame)
        stroke.Thickness = strokeWidth or 1
        stroke.Color = THEME.Stroke
        stroke.Transparency = 0.90 -- very subtle edge

        return stroke
    end

    local function makeText(parent, text, font, size, color, alignX, alignY)
        local t = make("TextLabel", {
            BackgroundTransparency = 1,
            Text = tostring(text or ""),
            Font = font or Enum.Font.Gotham,
            TextSize = size or 13,
            TextColor3 = color or THEME.Body,
            TextTransparency = 0,
    ...
    --========================
    -- Rayfield Shim Object
    --========================
    local Rayfield = {}
    Rayfield.__gui = nil
    Rayfield.__visible = true
    Rayfield.__notifyHolder = nil

    -- config (best-effort)
    Rayfield.__configEnabled = false
    Rayfield.__configFolder = "Axiom"
    Rayfield.__configFile = "axiom"
    Rayfield.__flags = {}  -- flag -> setter
    Rayfield.__values = {} -- flag -> last value
    ...
    function Rayfield:Destroy()
        ...
        self.__gui = nil
        self.__notifyHolder = nil
    end

    function Rayfield:ToggleUI()
        self.__visible = not self.__visible
        if self.__gui then
            self.__gui.Enabled = self.__visible
        end
    end

    Rayfield.Toggle = Rayfield.ToggleUI

    function Rayfield:Notify(payload)
        if not self.__notifyHolder then return end
        payload = payload or {}
        local title = tostring(payload.Title or "Notice")
        local content = tostring(payload.Content or "")
        local dur = tonumber(payload.Duration) or 3

        local card = make("Frame", {
            Size = UDim2.new(1, 0, 0, 74),
            BackgroundColor3 = THEME.Panel,
        }, self.__notifyHolder)
        applySubtleGlass(card, 12)

        local t = makeText(card, title, Enum.Font.GothamBold, 14, THEME.Title, Enum.TextXAlignment.Left,
            Enum.TextYAlignment.Top)
        t.Position = UDim2.new(0, 10, 0, 8)
        t.Size = UDim2.new(1, -20, 0, 18)

        local c = makeText(card, content, Enum.Font.Gotham, 13, THEME.Body, Enum.TextXAlignment.Left,
            Enum.TextYAlignment.Top)
        c.TextWrapped = true
        c.Position = UDim2.new(0, 10, 0, 28)
        c.Size = UDim2.new(1, -20, 1, -36)
        ...
        task.delay(dur, function()
            ...
            task.wait(0.2)
            pcall2(function() card:Destroy() end)
        end)
    end

    function Rayfield:SaveConfiguration()
        if not self.__configEnabled then return end
        if not fsOK() then return end
        ensureFolder(self.__configFolder)
        local path = self.__configFolder .. "/" .. self.__configFile .. ".json"
        pcall2(function()
            writefile(path, HttpService:JSONEncode(self.__values))
        end)
    end
    ...
    function Rayfield:LoadConfiguration()
        ...
        ensureFolder(self.__configFolder)
        local path = self.__configFolder .. "/" .. self.__configFile .. ".json"
        local ok, raw = pcall2(function() return readfile(path) end)
        if not ok or type(raw) ~= "string" then return end
        local ok2, tbl = pcall2(function() return HttpService:JSONDecode(raw) end)
        if not ok2 or type(tbl) ~= "table" then return end

        for flag, val in pairs(tbl) do
            self.__values[flag] = val
            local setter = self.__flags[flag]
            if setter then
                pcall2(setter, val)
            end
        end
	...
	function Rayfield:SetFlag(flag, value)
		local setter = self.__flags and self.__flags[flag]
			if setter then
				pcall(setter, value)
				return true
			end
    ...
    function Rayfield:CreateWindow(opts)
        ...
        self.__configEnabled = (cfg.Enabled == true)
        self.__configFolder  = tostring(cfg.FolderName or "Axiom")
        self.__configFile    = tostring(cfg.FileName or "axiom")
        ...
        -- Wrapper (no stroke) - we drag THIS
        local rootWrap = make("Frame", {
        ...
        -- Actual root (inset by 1px so stroke never clips)
        local root = make("Frame", {
            ...
            BorderSizePixel = 0,
            ClipsDescendants = false, -- IMPORTANT: root itself should not clip its stroke
        }, rootWrap)

        make("UICorner", { CornerRadius = UDim.new(0, HUB_RADIUS) }, root)
        make("UIStroke", {
            Thickness = 1,
		...
		uiScale.Scale = 0.5
		uiScale.Parent = rootWrap

		-- start invisible
		root.BackgroundTransparency = 1

		task.wait() -- let Roblox calculate sizes (important)
        ...
        local title = makeText(top, tostring(opts.Name or "Window"), Enum.Font.GothamBold, 16, THEME.Title,
            Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
        title.Position = UDim2.new(0, 10, 0, 6)
        title.Size = UDim2.new(1, -140, 0, 20)

        local sub = makeText(top, tostring(opts.LoadingSubtitle or ""), Enum.Font.Gotham, 13, THEME.Sub,
            Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
        sub.Position = UDim2.new(0, 9, 0, 20)
        ...
        -- Drag window (CLAMP rootWrap, not root)
        ...
        local function clampToScreen()
            ...
            -- rootWrap is centered at 0.5,0.5; clamp OFFSET only
            local w, h = rootWrap.AbsoluteSize.X, rootWrap.AbsoluteSize.Y
            local minX = -vp.X / 2 + w / 2 + 8
            local maxX = vp.X / 2 - w / 2 - 8
            local minY = -vp.Y / 2 + h / 2 + 8
            local maxY = vp.Y / 2 - h / 2 - 8

            local x = clamp(rootWrap.Position.X.Offset, minX, maxX)
            local y = clamp(rootWrap.Position.Y.Offset, minY, maxY)
            rootWrap.Position = UDim2.new(0.5, x, 0.5, y)
        end
        ...
        -- Body (clips content to window; safe because root stroke is outside this)
        ...
        applySubtleGlass(sidebar, HUB_RADIUS, 0) -- strokeWidth=0 => no stroke (less “double border” look)
        ...
        make("UIListLayout", { Padding = UDim.new(0, 8) }, notifyHolder)
        self.__notifyHolder = notifyHolder
        ...
        local TAB_FADE_IN  = TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local TAB_FADE_OUT = TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local TAB_SLIDE    = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

local function selectTab(name)
	if activeTab == name then return end

	local oldPage = activeTab and pagesByName[activeTab]
	local newPage = pagesByName[name]
	...
	for n, btn in pairs(tabButtons) do
		btn.BackgroundTransparency = (n == name) and 0.06 or 0.12
	end

	-- fade out old
	if oldPage and pageCG[oldPage] then
		local oldCg = pageCG[oldPage]
		TweenService:Create(oldCg, TAB_FADE_OUT, { GroupTransparency = 1 }):Play()
		task.delay(TAB_FADE_OUT.Time, function()
	...
	local newCg = pageCG[newPage]
	if newCg then newCg.GroupTransparency = 1 end

	-- subtle slide-in
	local startPos = newPage.Position
	newPage.Position = startPos + UDim2.fromOffset(6, 0)

	if newCg then
		TweenService:Create(newCg, TAB_FADE_IN, { GroupTransparency = 0 }):Play()
	end
	TweenService:Create(newPage, TAB_SLIDE, { Position = startPos }):Play()
end
        ...
        -- Window object exposed to your script
        local Window = {}
        ...
        Window.ModifyTheme = function(_themeName)
            -- no-op
        end
        ...
        function Window:CreateTab(tabName, _icon)
            ...
            -- tab button (darker, readable)
            local btn = make("TextButton", {
                BackgroundColor3 = THEME.Panel2,
                BackgroundTransparency = 0.12,
                BorderSizePixel = 0,
                Size = UDim2.new(1, 0, 0, 34),
                Text = "", -- IMPORTANT: we render our own label so we can align icon+text
                AutoButtonColor = true,
            }, tabList)

            applySubtleGlass(btn, HUB_RADIUS, 0) -- keep your current styling

            -- layout inside the button: icon then text
            ...
            -- icon (supports: number id / "123" / "rbxassetid://123" / lucide name "info")
            local iconObj = makeIcon(inner, _icon, 16, THEME.Title)
            ...
            -- if no icon resolved, tighten spacing (no empty gap)
            if not iconObj then
            ...
            local function rowFrame(height)
                ...
                applySubtleGlass(f, 12)
                return f
            end

            function Tab:CreateSection(titleText)
                local t = makeText(page, tostring(titleText or ""), Enum.Font.GothamBold, 14, THEME.Title,
                    Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
            ...
            function Tab:CreateParagraph(opts)
                ...
                local t = makeText(f, tostring(opts.Title or ""), Enum.Font.GothamBold, 14, THEME.Title,
                ...
                -- Content (auto height, wrapped)
                local c = makeText(f, tostring(opts.Content or ""), Enum.Font.Gotham, 13, THEME.Body,
                    Enum.TextXAlignment.Left, Enum.TextYAlignment.Top)
                ...
                -- Object API (compatible with your existing Set/Update helpers)
                local obj = {}

                function obj:Set(newOpts)
                    newOpts = newOpts or {}
                    if newOpts.Title ~= nil then t.Text = tostring(newOpts.Title) end
                    if newOpts.Content ~= nil then c.Text = tostring(newOpts.Content) end
                end

                obj.Update = function(_, v)
                    c.Text = tostring(v or "")
                end
                obj.Refresh = obj.Update
                obj.SetValue = obj.Update
            ...
            function Tab:CreateToggle(opts)
                opts = opts or {}
                local cur = (opts.CurrentValue == true)
                local f = rowFrame(44)

                local name = makeText(f, tostring(opts.Name or "Toggle"), Enum.Font.GothamSemibold, 14, THEME.Title,
                    Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
                name.Position = UDim2.new(0, 12, 0, 0)
                name.Size = UDim2.new(1, -90, 1, 0)

                local pill = make("Frame", {
                    BackgroundColor3 = cur and Color3.fromRGB(80, 200, 120) or Color3.fromRGB(60, 60, 60),
                    BorderSizePixel = 0,
                    Size = UDim2.fromOffset(46, 24),
                    Position = UDim2.new(1, -62, 0.5, -12),
                }, f)
                make("UICorner", { CornerRadius = UDim.new(1, 0) }, pill)
                make("UIStroke", { Transparency = 0.85, Color = Color3.fromRGB(255, 255, 255) }, pill)
                ...
                local function set(v)
                    cur = (v == true)
                    local goalDot = cur and UDim2.new(1, -21, 0.5, -9) or UDim2.new(0, 3, 0.5, -9)
                    local goalCol = cur and Color3.fromRGB(80, 200, 120) or Color3.fromRGB(60, 60, 60)

                    pcall2(function()
                        TweenService:Create(dot, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                            { Position = goalDot }):Play()
                        TweenService:Create(pill, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                            { BackgroundColor3 = goalCol }):Play()
                    end)

                    if opts.Flag then Rayfield.__values[opts.Flag] = cur end
                    if type(opts.Callback) == "function" then opts.Callback(cur) end
                    Rayfield:SaveConfiguration()
                end

				if opts.Flag then
					Rayfield.__flags[opts.Flag] = set
					Rayfield.__values[opts.Flag] = cur
				end
                ...
                if opts.Flag then Rayfield.__flags[opts.Flag] = set end
                return { Set = set, Update = set, Refresh = set }
            end
            ...
            function Tab:CreateSlider(opts)
                ...
                local label = makeText(f, tostring(opts.Name or "Slider"), Enum.Font.GothamSemibold, 14, THEME.Title,
                    Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
                label.Position = UDim2.new(0, 12, 0, 8)
                label.Size = UDim2.new(0.65, 0, 0, 18)

                local valueLabel = makeText(f, tostring(cur) .. (opts.Suffix and (" " .. opts.Suffix) or ""),
                    Enum.Font.Gotham, 13, THEME.Body, Enum.TextXAlignment.Right, Enum.TextYAlignment.Center)
                ...
                make("UIStroke", { Transparency = 0.90, Color = Color3.fromRGB(255, 255, 255) }, bar)

                local fill = make("Frame", {
                    BackgroundColor3 = Color3.fromRGB(90, 160, 240),
                    BorderSizePixel = 0,
                ...
                local function set(v)
                    ...
                    cur = clamp(v, minV, maxV)
                    fill.Size = UDim2.new((cur - minV) / (maxV - minV), 0, 1, 0)
                    valueLabel.Text = tostring(cur) .. (opts.Suffix and (" " .. opts.Suffix) or "")
                    if opts.Flag then Rayfield.__values[opts.Flag] = cur end
                    if type(opts.Callback) == "function" then opts.Callback(cur) end
                    Rayfield:SaveConfiguration()
                end
            ...
            function Tab:CreateDropdown(opts)
                ...
                local baseH = 52
                local f = rowFrame(baseH)

                local label = makeText(f, tostring(opts.Name or "Dropdown"), Enum.Font.GothamSemibold, 14, THEME.Title,
                    Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
                label.Position = UDim2.new(0, 12, 0, 8)
                ...
                applySubtleGlass(menu, 10)

                local list = make("UIListLayout", { Padding = UDim.new(0, 6) }, menu)
                make("UIPadding", {
                ...
                local function rebuild()
                    ...
                    for _, opt in ipairs(options) do
                        ...
                        make("UICorner", { CornerRadius = UDim.new(0, 8) }, ob)
                        ob.MouseButton1Click:Connect(function()
                            current = opt
                            btn.Text = tostring(current)
                            closeMenu()
                            if opts.Flag then Rayfield.__values[opts.Flag] = current end
                            if type(opts.Callback) == "function" then opts.Callback(current) end
                            Rayfield:SaveConfiguration()
                        end)
                    end
                    menu.Size = UDim2.new(1, -24, 0, list.AbsoluteContentSize.Y + 16)
                end
                ...
                btn.MouseButton1Click:Connect(function()
                    ...
                    if open then
                        rebuild()
                        menu.Visible = true
                        -- NOTE: page is clipped to window, so dropdown won't escape main frame
                        f.Size = UDim2.new(1, 0, 0, baseH + menu.Size.Y.Offset + 8)
                    else
                        closeMenu()
                    end
                end)
                ...
                local function setValues(vals)
                    options = vals or {}
                    if #options > 0 and not table.find(options, current) then
                        current = options[1]
                        btn.Text = tostring(current)
                    end
                end
                ...
                if opts.Flag then
					Rayfield.__flags[opts.Flag] = function(v) setCurrent(v) end
					Rayfield.__values[opts.Flag] = current
				end


                return {
                    Set = function(_, v)
                        if type(v) == "table" then setValues(v) else setCurrent(v) end
                    end,
                    Refresh = function(_, v) setValues(v) end,
                    Update = function(_, v) setValues(v) end,
                    SetOptions = function(_, v) setValues(v) end,
                    SetValues = function(_, v) setValues(v) end,
                    SetCurrentOption = function(_, v) setCurrent(v) end,
                    SetValue = function(_, v) setCurrent(v) end,
                }
            end

            -- Optional compatibility methods
            function Tab:CreateLabel(text, _icon, _color, _ignoreTheme)
                local f = make("Frame", {
                    BackgroundColor3 = THEME.Panel,
                    BackgroundTransparency = 0.08,
                    BorderSizePixel = 0,
                ...
                local l = makeText(f, tostring(text or ""), Enum.Font.Gotham, 13, THEME.Body, Enum.TextXAlignment.Left,
                    Enum.TextYAlignment.Center)
                l.Position = UDim2.new(0, 12, 0, 0)
                l.Size = UDim2.new(1, -24, 1, 0)

                local obj = {}
                function obj:Set(v) l.Text = tostring(v or "") end
            ...
            function Tab:CreateInput(opts)
                ...
                local label = makeText(f, tostring(opts.Name or "Input"), Enum.Font.GothamSemibold, 14, THEME.Title,
                    Enum.TextXAlignment.Left, Enum.TextYAlignment.Center)
                label.Position = UDim2.new(0, 12, 0, 8)
...
local CustomRayfieldFactory = BuildAxiomRayfield

--========================
-- Imports
--========================
local Rayfield
if USE_CUSTOM_GUI then
    Rayfield = CustomRayfieldFactory()
else
...
local function bindLocalDeath()
    if localDiedConn then
        localDiedConn:Disconnect(); localDiedConn = nil
    end
    local hum
...
Tabs.Player:CreateButton({
    ...
    Callback = function()
		...
		Rayfield:SetFlag("WalkSpeed", 16)
		Rayfield:SetFlag("JumpPower", 50)
		Rayfield:SetFlag("FOV", 70)
		Rayfield:SetFlag("HipHeightEnabled", false) -- if you want it toggled off
		Rayfield:SetFlag("HipHeight", 0)
...
local function stopSpider()
    if spiderConn then
        spiderConn:Disconnect(); spiderConn = nil
    end
    pcall(function()
        local hum = getHumanoid()
...
Tabs.Movement:CreateButton({
    ...
    Callback = function()
		...
		Rayfield:SetFlag("Noclip", false)
		Rayfield:SetFlag("InfiniteJump", false)
		Rayfield:SetFlag("Fly", false)
		Rayfield:SetFlag("Gravity", 196.2)
		Rayfield:SetFlag("FlySpeed", 60)
...
local function disconnectFollowDied()
    if followDiedConn then
        followDiedConn:Disconnect(); followDiedConn = nil
    end
end
...
track(Teams.ChildAdded:Connect(function()
    refreshTeamDropdown(true); refreshTargetDropdown(true)
end))
track(Teams.ChildRemoved:Connect(function()
    refreshTeamDropdown(true); refreshTargetDropdown(true)
end))
...
track(Players.PlayerRemoving:Connect(function(plr)
    if teamConns[plr] then
        teamConns[plr]:Disconnect(); teamConns[plr] = nil
    end
end))
...
Tabs.Teleport:CreateButton({
    ...
    Callback = function()
        ...
        if ok then
            notify("Waypoints", "Imported successfully.", 3)
        else
            notify("Waypoints", "Import failed: " .. tostring(err), 5)
        end
    end
})
...
Tabs.Visual:CreateToggle({
    ...
    CurrentValue = state.removeEffects,
    Flag = "RemoveEffects",
    Callback = function(
        on)
        state.removeEffects = on; setRemoveEffects(on)
    end
})
if premium() then
...
Tabs.Visual:CreateToggle({
    Name = "No Weather (Fog/Atmosphere)",
    CurrentValue = state.noWeather,
    Flag = "NoWeather",
    Callback = function(
        on)
        state.noWeather = on; setNoWeather(on)
    end
})
Tabs.Visual:CreateToggle({
    Name = "No Particles",
    CurrentValue = state.noParticles,
    Flag = "NoParticles",
    Callback = function(
        on)
        state.noParticles = on; setNoParticles(on)
    end
})
...
local function stopFreecam()
    ...
    if freecamConn then
        freecamConn:Disconnect(); freecamConn = nil
    end
...
Tabs.Visual:CreateButton({
    Name = "Reset Camera",
    Callback = function()
        stopFreecam(); notify("Camera", "Reset.", 2)
    end
})
...
local function cleanupESPPlayer(plr)
    ...
    if h then
        h:Destroy(); highlightsByPlayer[plr] = nil
    end
    local bb = billboardsByPlayer[plr]
    if bb then
        bb:Destroy(); billboardsByPlayer[plr] = nil
    end
...
local function startESP()
    ...
    espConn = RunService.RenderStepped:Connect(function(dt)
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if not shouldShowESP(plr, dist) then
                cleanupESPPlayer(plr)
                continue
            end
            ...
            if state.espShowNames or state.espShowDistance or state.espShowHealth or state.espShowTeam then
                ...
                if billboardsByPlayer[plr] then
                    billboardsByPlayer[plr]:Destroy(); billboardsByPlayer[plr] = nil
                end
            end
...
local function startDrawingESP()
    ...
    drawingConn = RunService.RenderStepped:Connect(function()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if state.espTeamCheck and plr.Team == LocalPlayer.Team then
                dremove(plr)
                continue
            end
...
local ServerHop = (function()
    ...
    local function fetch(placeId, limit)
        ...
        if cursor == "" then
            ...
            url = ("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Asc&limit=%d&cursor=%s"):format(
                placeId, limit, cursor)
        end
...
track(RunService.Heartbeat:Connect(function(dt)
    ...
    if premium() then
        ...
        local hp = (hum and hum.MaxHealth) and
            ("%d/%d"):format(math.floor(hum.Health + 0.5), math.floor(hum.MaxHealth + 0.5)) or "-"
